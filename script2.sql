...
-- -----------------------------------------------------
-- Vista Amarres Ocupados
-- -----------------------------------------------------

CREATE VIEW amarres_ocupados
AS SELECT ta.Desc_Tipo_Amarre, tn.Desc_Tipo_Navío, s.FechaHora_Salida_Est, se.Desc_Servicio
FROM amarres a, puertos p, navíos n, tipos_amarres ta, solicitudes s, tipos_navíos tn, servicios se, servicios_solicitados ss
WHERE (a.Cod_Tipo_Amarre=ta.Cod_Tipo_Amarre) AND (p.Cod_Puerto=s.Cod_Puerto) AND  
(n.Cod_Tipo_Navío= tn.Cod_Tipo_Navío) AND (FechaHora_Arribo_Est>='2020-01-10') AND 
(FechaHora_Salida_Est<='2020-12-25') AND (se.Cod_Servicio=ss.Cod_Servicio) AND 
(ss.Cod_Solicitud=s.Cod_Solicitud)

-- -----------------------------------------------------
-- USUARIO ADMINISTRADOR - para que se conecte localmente a la base de datos y tenga todos los permisos sobre la base de datos
-- -----------------------------------------------------

DROP USER 'JuanRodriguez'@'localhost';
CREATE USER 'JuanRodriguez'@'localhost' IDENTIFIED BY 'admin';
GRANT ALL ON tpbd2. * TO 'JuanRodriguez'@'localhost';


-- -----------------------------------------------------
-- USUARIO GERENTE - para que se conecte remotamente a la base de datos y tenga todos los permisos 
-- -----------------------------------------------------

DROP USER 'RamiroFuentes'@'%';
CREATE USER 'RamiroFuentes'@'%' IDENTIFIED BY 'gerent';
GRANT ALL ON tpbd2. * TO 'RamiroFuentes'@'%';


-- -----------------------------------------------------
-- USUARIO CONSULTA - para que se conecte remotamente a la base de datos y tenga permisos de consulta 
-- -----------------------------------------------------
DROP USER 'consulta'@'%';
CREATE USER 'consulta'@'%' IDENTIFIED BY 'consult';
GRANT SELECT ON tpbd2. * TO 'consulta'@'%';

-- -----------------------------------------------------
-- USUARIO CONSULTA EJEMPLO - para que se conecte remotamente a la base de datos y tenga permisos de consulta, inserción, borrado y actualización
-- -----------------------------------------------------
DROP USER 'consulta_ej'@'%';
CREATE USER 'consulta_ej'@'%' IDENTIFIED BY 'consult_ej';
GRANT SELECT,INSERT,DELETE, ALTER ON tpbd2. * TO 'consulta_ej'@'%';
